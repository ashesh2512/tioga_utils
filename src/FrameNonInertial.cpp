...
void FrameNonInertial::update_coordinates_velocity(const double time)
{
  ...
  for (auto b: bkts) {
    for (size_t in=0; in < b->size(); in++) {
      ...
      MotionBase::trans_mat_type trans_mat = compute_transformation(time,oldxyz);
      ...
      {
        MotionBase::threeD_vec_type mm_vel = mm->compute_velocity(time,trans_mat,xyz);
...
MotionBase::trans_mat_type FrameNonInertial::compute_transformation(
  const double time,
  const double* xyz)
{
  // all non-inertial frame motions are based off of the reference frame
  MotionBase::trans_mat_type comp_trans_mat_ = ref_frame_;
  ...
  for (auto& mm: meshMotionVec_)
  {
    ...
    comp_trans_mat_ = mm->add_motion(mm->get_trans_mat(),comp_trans_mat_);
  }
