  ...
  for (auto b: bkts) {
    for (size_t in=0; in < b->size(); in++) {
      ...
      for (int d = 0; d < ndim; d++) {
        xyz[d] = inertial_frame_[d][0]*oldxyz[0]
                +inertial_frame_[d][1]*oldxyz[1]
                +inertial_frame_[d][2]*oldxyz[2]
                +inertial_frame_[d][3];
...
void FrameInertial::compute_transformation(const double time)
{
  for (auto& mm: meshMotionVec_)
  {
    ...
    inertial_frame_ = mm->add_motion(mm->get_trans_mat(),inertial_frame_);
  }
}
